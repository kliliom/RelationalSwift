...
func check(_ block: @autoclosure () -> Int32, db: OpaquePointer? = nil, is result: Int32) throws {
    ...
    if let db {
        throw RelationalSwiftError(message: String(cString: sqlite3_errmsg(db)), code: code)
    } else {
        throw RelationalSwiftError(message: String(cString: sqlite3_errstr(code)), code: code)
    }
}
